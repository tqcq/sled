...
template<typename T>
inline constexpr remove_reference_t<T> &&
move(T &&t) noexcept
{
...
invoke(R T::*f, Arg &&arg, Args &&...args)
        ...
        namespace detail
{
    ...
    template<typename F, typename... Args>
    struct invoke_result<void_t<decltype(lib::invoke(std::declval<F>(),
                                                     std::declval<Args>()...))>,
                         F, Args...>
        : identity<decltype(lib::invoke(std::declval<F>(),
                                        std::declval<Args>()...))> {};
...
template<typename T>
inline constexpr T *
addressof(T &arg, std::true_type) noexcept
{
...
template<typename T>
inline constexpr T *
addressof(T &arg, std::false_type) noexcept
{
    return &arg;
}
...
#else
inline constexpr std::size_t
find_index_impl(std::size_t result, std::size_t)
{
    return result;
}
...
template<typename... Traits>
inline constexpr Trait
common_trait_impl(Trait result, Trait t, Traits... ts)
{
...
#define MPARK_DEFAULT(I)                                                       \
    dispatcher<(I < lib::decay_t<V>::size()), R, ITs...>::template dispatch<   \
        I>(lib::forward<F>(f),                                                 \
           lib::forward<typename ITs::type>(visited_vs)...,                    \
           lib::forward<V>(v), lib::forward<Vs>(vs)...)
...
#undef MPARK_DISPATCH
        ...
        template<std::size_t B, typename F, typename V, typename... Vs>
        MPARK_ALWAYS_INLINE static constexpr R
        dispatch_at(std::size_t index, F &&f, V &&v, Vs &&...vs)
        {
...
#define MPARK_DEFAULT(I)                                                       \
    dispatcher<(I < lib::decay_t<V>::size()), R>::template dispatch_at<I>(     \
        index, lib::forward<F>(f), lib::forward<V>(v),                         \
        lib::forward<Vs>(vs)...)
...
#undef MPARK_DISPATCH_AT
    ...
    struct make_fmatrix_impl {
        ...
        impl(Is, lib::index_sequence<Js...>, Ls... ls)
        {
    ...
    inline static constexpr AUTO make_value_visitor(Visitor &&visitor)
        ...
        : template<typename Visitor, typename... Vs>
          ...
          visit_alt(Visitor &&visitor, Vs &&...vs) DECLTYPE_AUTO_RETURN(
              alt::visit_alt(lib::forward<Visitor>(visitor),
...
template<std::size_t I, typename V>
inline constexpr /* auto * */ AUTO
generic_get_if(V *v) noexcept
...
struct convert_to_bool {
    ...
    inline constexpr bool operator()(Lhs &&lhs, Rhs &&rhs) const
    {
        ...
        return lib::invoke(RelOp{}, lib::forward<Lhs>(lhs),
                           lib::forward<Rhs>(rhs));
    }
};
}// namespace detail
...
operator<=(const variant<Ts...> &lhs, const variant<Ts...> &rhs)
{
    ...
    return lhs.valueless_by_exception()
        || (!rhs.valueless_by_exception()
            && (lhs.index() < rhs.index()
                || (lhs.index() == rhs.index()
                    && variant::visit_value_at(lhs.index(), less_equal{}, lhs,
                                               rhs))));
#endif
}
...
operator>=(const variant<Ts...> &lhs, const variant<Ts...> &rhs)
{
    ...
    return rhs.valueless_by_exception()
        || (!lhs.valueless_by_exception()
            && (lhs.index() > rhs.index()
                || (lhs.index() == rhs.index()
                    && variant::visit_value_at(lhs.index(), greater_equal{},
                                               lhs, rhs))));
#endif
}
...
inline constexpr bool
all(std::initializer_list<bool> bs)
{
    for (bool b : bs) {
        if (!b) { return false; }
    }
...
template<std::size_t N>
inline constexpr bool
all_impl(const lib::array<bool, N> &bs, std::size_t idx)
{
...
template<typename Visitor, typename... Vs>
inline constexpr DECLTYPE_AUTO
visit(Visitor &&visitor, Vs &&...vs) DECLTYPE_AUTO_RETURN(
